<x-app-layout>
    <x-slot name="header">
        <p class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            <div class="flex items-center font-bold">Master<i class="fi fi-rr-caret-right mt-1"></i> Jadwal<i
                    class="fi fi-rr-caret-right mt-1"></i> <span class="text-amber-100">Ruang</span></div>
        </p>
    </x-slot>
                ...
                <div class="w-full md:w-3/12 p-3">
                    <div class="bg-white w-full dark:bg-gray-800 overflow-hidden shadow-xl border border-gray-200 rounded-3xl">
                        <div class="p-6 text-gray-900 dark:text-gray-100">
                            ...
                            <form action="{{ route('ruang.store') }}" method="post" id="ruangForm">
                                @csrf
                                <div class="p-4 rounded-xl">
                                    <div class="mb-5">
                                        ...
                                        <input type="text" id="ruang" name="ruang"
                                            ...
                                            placeholder="Masukan Nama Ruang disini ..." oninput="this.value = this.value.toUpperCase();"/>
                                        <p id="error-ruang" class="mt-2 text-sm text-red-500 hidden">Ruang wajib diisi.
                                        </p>
                                    </div>
                                    <button type="submit"
            ...
            <div class="w-full md:w-1/4 relative bg-white rounded-lg shadow mx-5">
                ...
                <form method="POST" id="formSourceModal">
                    ...
                    <div class="flex flex-col  p-4 space-y-6">
                        ...
                        <div>
                            <label for="text" class="block mb-2 text-sm font-medium text-gray-900">Ruang <span
                                    class="text-red-500">*</span></label>
                            <input type="text" id="ruangs" name="ruang"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                id="" placeholder="Masukan ruang disini..." oninput="this.value = this.value.toUpperCase();">
                        </div>
                    </div>
    ...
    <script>
        const form = document.getElementById('ruangForm');
                    ...
                    render: (data) => {
                        let editUrl =
                            `<button type="button" data-id="${data.id}"
                                                        data-modal-target="sourceModal" data-ruang="${data.ruang}"
                        ...
                        let deleteUrl =
                            `<button onclick="return ruangDelete('${data.id}','${data.ruang}')" class="border-2 border-dashed border-red-500 text-red-500 hover:bg-red-100 px-3 py-1 rounded-xl h-10 w-10 text-xs"><i class="fas fa-trash"></i></button>`;
        ...
        const editSourceModal = (button) => {
            ...
            document.getElementById('ruangs').value = ruang;
        ...
        const ruangDelete = async (id, ruang) => {
            Swal.fire({
                title: `Apakah Anda yakin?`,
                text: `Data ruang ${ruang} akan dihapus secara permanen!`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    await axios.post(`/ruang/${id}`, {
                        ...
                        .then(function(response) {
                            Swal.fire({
                                title: 'Terhapus!',
                                text: `Data ruang ${ruang} berhasil dihapus.`,
                                icon: 'success',
