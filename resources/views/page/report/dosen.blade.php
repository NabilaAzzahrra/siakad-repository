<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex items-center font-bold">Report<i class="fi fi-rr-caret-right mt-1"></i> <span
                class="text-amber-100">Presensi Dosen</span></div>
        </p>
    </x-slot>
                            ...
                            <div class="pt-3 w-full" style="width:100%;overflow-x:auto;">
                                <div class="flex flex-col sm:flex-row justify-between items-center mb-4">
                                    <div class="md:mt-0 sm:flex sm:space-x-4 w-full">
                                        <!-- Form untuk entries -->
                                        <x-show-entries :route="route('report_dosen.index')" :search="request()->search">
                                        </x-show-entries>
                                    </div>
                                    <div class="sm:ml-16 sm:mt-0 sm:flex sm:space-x-4 sm:flex-none">
                                        <form action="{{ route('report_dosen.index') }}" method="GET"
                                    ...
                                    <table
                                        ...
                                        <tbody id="jadwalRegulerTable">
                                            @php
                                                $no = $dosen->firstItem();
                                            @endphp
                                            @forelse($dosen as $key => $i)
                                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                                    <td class="px-6 py-4 text-center bg-gray-100">
                                                        {{ $dosen->perPage() * ($dosen->currentPage() - 1) + $key + 1 }}
                                                    </td>
                                                    <th scope="row"
                                                    ...
                                                    <td class="px-6 py-4 bg-gray-100 text-center">
                                                        <a href="{{ route('report_dosen.show', $i->id) }}"
                                ...
                                <div class="mt-4">
                                    @if ($dosen->hasPages())
                                        {{ $dosen->appends(request()->query())->links('vendor.pagination.custom') }}
                                    @else
                                        <div class="flex items-center justify-between">
                                            <nav class="flex justify-start">
                                                <div class="text-sm flex gap-1">
                                                    ...
                                                    <div>to</div>
                                                    <div class="font-bold">{{ count($dosen) }}</div>
                                                    <div>of</div>
                                                    <div class="font-bold">{{ count($dosen) }}</div>
                                                    <div>entries</div>
                                                </div>
